...
const getCCP = async (org) => {
    let ccpPath;
    if (org == "bigini") {
        ccpPath = path.resolve(__dirname, '..', 'config', 'connection-bigini.json');

    } else if (org == "lattanzi") {
        ccpPath = path.resolve(__dirname, '..', 'config', 'connection-lattanzi.json');
    } else
        return null
...
const getCaUrl = async (org, ccp) => {
    let caURL;
    if (org == "bigini") {
        caURL = ccp.certificateAuthorities['ca.bigini.halonetwork.com'].url;
...
const getWalletPath = async (org) => {
    let walletPath;
    if (org == "bigini") {
        walletPath = path.join(process.cwd(), 'bigini-wallet');
...
const getAffiliation = async (org) => {
    return org == "bigini" ? 'bigini.department1' : 'lattanzi.department1'
}
...
const getRegisteredUser = async (username, userOrg, isJson) => {
    ...
    try {
        if (username == "superuser") {
            ...
            secret = await ca.register({ affiliation: 'bigini.department1', enrollmentID: username, role: 'client', attrs: [{ name: 'role', value: 'admin', ecert: true }] }, adminUser);
